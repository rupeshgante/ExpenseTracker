...
download.addEventListener('click',()=>{
    console.log('clicked');
    axios.get('http://3.94.195.113/user/download',{headers:{Authentication:token}})
        .then(res=>{
            if(res.status===200){
...
window.addEventListener('DOMContentLoaded',()=>{
    ...
    axios.get('http://3.94.195.113/user/getexpense',{headers:{Authentication:token}})
    .then(res=>{
        console.log(res);        
...
previous.addEventListener('click',()=>{
    axios.get('http://3.94.195.113/user/previousfiles',{headers:{Authentication:token}})
       .then(res=>{
        console.log(res);
